        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-8">
            {featuredPublications.map((pub) => (
              <article key={pub.id} className="bg-white rounded-lg shadow-lg p-8 hover:shadow-xl transition-shadow">
                <div className="flex items-start space-x-6">
                  <div className="w-16 h-16 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-2xl">{pub.image}</span>
                  </div>
                  
                  <div className="flex-grow">
                    <div className="flex items-center mb-3">
                      <span className="inline-block bg-green-100 text-green-800 text-xs px-3 py-1 rounded-full font-medium mr-3">
                        {pub.category}
                      </span>
                    ...
                    <p className="text-gray-700 mb-3">{pub.authors}</p>
                    
                    <div className="flex items-center mb-4 text-sm">
                      <span className="font-medium text-gray-700 mr-2">Jurnal:</span>
                      <span className="text-green-600 font-medium mr-4">{pub.journal}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recentPublications.map((pub) => (
              <article key={pub.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
                <div className="flex items-start space-x-4">
                  <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-xl">{pub.image}</span>
                  </div>
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-green-600 text-sm font-medium">{pub.venue}</p>
                  </div>
                </div>
              </article>
            ))}
              ...
              <div key={index} className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-green-600 font-medium">{collab.country}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">{collab.projects}</div>
                    <div className="text-sm text-gray-600">Proyek Aktif</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">{collab.publications}</div>
                    <div className="text-sm text-gray-600">Publikasi Bersama</div>
                  </div>
                </div>
              </div>
            ))}
